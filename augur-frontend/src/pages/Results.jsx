...
function transformPath(path, index, initialCash) {
  ...
  return {
    id: LABELS[index], pathId: path.id, score: Math.round(path.score),
    progression, timeLabels, timeValues, totalHorizon, final,
    cashGrowth, minRunway, riskScore, nodes: path.nodes, initialCash,
  }
}
...
function TreeAnimation({ onComplete, dataReady }) {
  ...
  useEffect(() => {
    ...
    const L3 = [cx-420,cx-360,cx-295,cx-230,cx-165,cx-95,cx-30,cx+30,cx+95,cx+165,cx+230,cx+295,cx+360,cx+420,cx+470,cx-470].map((x,i) => ({ x, y: 308, id: 16+i }))
    const L4 = [cx-450,cx-410,cx-365,cx-320,cx-275,cx-225,cx-175,cx-125,cx-75,cx-25,cx+25,cx+75,cx+125,cx+175,cx+225,cx+275,cx+325,cx+375,cx+415,cx+455].map((x,i) => ({ x, y: 398, id: 32+i }))
    const allNodes = [...L0,...L1,...L2,...L3,...L4]

    const allEdges = [
      ...L1.map(n => ({ from: L0[0], to: n })),
      {from:L1[0],to:L2[0]},{from:L1[0],to:L2[1]},{from:L1[1],to:L2[2]},{from:L1[1],to:L2[3]},
      {from:L1[2],to:L2[4]},{from:L1[2],to:L2[5]},{from:L1[3],to:L2[6]},{from:L1[3],to:L2[7]},
      {from:L1[4],to:L2[8]},{from:L1[4],to:L2[9]},
      {from:L2[0],to:L3[0]},{from:L2[0],to:L3[15]},{from:L2[1],to:L3[1]},{from:L2[1],to:L3[2]},
      {from:L2[2],to:L3[3]},{from:L2[3],to:L3[4]},{from:L2[3],to:L3[5]},{from:L2[4],to:L3[6]},
      {from:L2[5],to:L3[7]},{from:L2[5],to:L3[8]},{from:L2[6],to:L3[9]},{from:L2[7],to:L3[10]},
      {from:L2[7],to:L3[11]},{from:L2[8],to:L3[12]},{from:L2[9],to:L3[13]},{from:L2[9],to:L3[14]},
      {from:L3[0],to:L4[0]},{from:L3[0],to:L4[1]},{from:L3[1],to:L4[2]},{from:L3[2],to:L4[3]},
      {from:L3[2],to:L4[4]},{from:L3[3],to:L4[5]},{from:L3[4],to:L4[6]},{from:L3[4],to:L4[7]},
      {from:L3[5],to:L4[8]},{from:L3[6],to:L4[9]},{from:L3[6],to:L4[10]},{from:L3[7],to:L4[11]},
      {from:L3[8],to:L4[12]},{from:L3[8],to:L4[13]},{from:L3[9],to:L4[14]},{from:L3[10],to:L4[15]},
      {from:L3[10],to:L4[16]},{from:L3[11],to:L4[17]},{from:L3[12],to:L4[18]},{from:L3[13],to:L4[19]},
      {from:L3[14],to:L4[18]},{from:L3[15],to:L4[0]},
    ]

    const highlightPaths = [
      [L0[0],L1[0],L2[1],L3[2], L4[4] ],
      [L0[0],L1[2],L2[4],L3[6], L4[9] ],
      [L0[0],L1[4],L2[9],L3[13],L4[19]],
    ]

    const nodeRadius = n => n.id === 0 ? 9 : n.id <= 5 ? 6 : 4.5
    ...
    function draw(ts) {
      ...
      if (highlightStart !== null) {
        ...
        if (highlightRaw > 0.80) {
          ...
          highlightPaths.forEach((path, pi) => {
            ...
            ctx.fillStyle = accent; ctx.beginPath(); ctx.roundRect(bx, by, bw, bh, 9); ctx.fill()
            ctx.fillStyle = color; ctx.textAlign = "center"; ctx.fillText(label, end.x, by+13)
            ctx.globalAlpha = 1
          })
        }
      }
      ...
      if (highlightStart === null) {
        ...
        ctx.globalAlpha = fadeIn; ctx.fillStyle = "#64748b"
        ctx.fillText(`${frozenCount.toLocaleString()} paths evaluated · top 3 strategies selected`, cx, H - 20)
        ctx.globalAlpha = 1
      }
  ...
  return (
    <canvas ref={canvasRef} width={980} height={460}
...
function DivergenceChart({ paths, activeIndex }) {
  ...
  const rafRef = useRef(), startRef = useRef()
  ...
  const pxByMonth = m => PAD.left + (m / totalHorizon) * innerW
  const py = v => PAD.top + innerH - ((v - minV) / (maxV - minV)) * innerH

  const animPts = (prog, path) => {
    const { progression: data, timeValues: times } = path
    const floatIdx = prog * (data.length - 1), full = Math.floor(floatIdx), frac = floatIdx - full
      ...
      {[0,0.25,0.5,0.75,1].map((t, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={PAD.left-8} y={y+4} textAnchor="end" fill="#cbd5e1" fontSize="10" fontFamily="monospace">{fmt(maxV - t*(maxV-minV))}</text>
          </g>
        )
      })}
      {paths[0]?.timeLabels.map((label, i) => (
        <text key={i} x={pxByMonth(paths[0].timeValues[i])} y={H-4} textAnchor="middle" fill="#cbd5e1" fontSize="10" fontFamily="monospace">{label}</text>
      ))}
      {paths.map((path, si) => {
        ...
        return (
          <g key={path.id}>
            ...
            <path d={pathD} fill="none" stroke={color} strokeWidth={isActive?2.5:1.5} strokeLinecap="round" strokeLinejoin="round" opacity={isActive?1:0.2} />
            {pts.map((p, i) => i < path.progression.length && (
              <circle key={i} cx={p[0]} cy={p[1]} r={isActive?4:2.5} fill={isActive?color:"white"} stroke={color} strokeWidth={isActive?0:1.5} opacity={isActive?1:0.35} />
            ))}
            {progress > 0.92 && pts.length >= path.progression.length && (
              <text x={pxByMonth(totalHorizon)+8} y={py(path.progression[path.progression.length-1])+4} fill={color} fontSize="11" fontWeight="700" fontFamily="monospace" opacity={isActive?1:0.35}>
...
function StrategyCard({ path, rank, isFront, offset, onClick, onDetail }) {
  ...
  return (
    <div onClick={onClick} style={{
      ...
      opacity:isFront?1:0.5, filter:isFront?"none":"blur(1.5px)",
      zIndex:isFront?10:5-Math.abs(offset), transition:"all 0.5s cubic-bezier(0.34,1.56,0.64,1)",
      cursor:isFront?"default":"pointer", borderRadius:"16px", background:"white",
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"18px" }}>
        <div>
          <div style={{ display:"inline-flex", alignItems:"center", background:accent, borderRadius:"20px", padding:"3px 10px", marginBottom:"8px" }}>
            <span style={{ fontSize:"10px", fontWeight:"700", color, letterSpacing:"1px", fontFamily:"monospace" }}>#{rank+1} RANKED</span>
          </div>
          <div style={{ fontSize:"17px", fontWeight:"800", color:"#0f172a", letterSpacing:"-0.5px" }}>{path.id}</div>
        </div>
        <ScoreArc score={path.score} color={color} />
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"8px", marginBottom:"18px" }}>
        {metrics.map(({ label, value, hi }) => (
          <div key={label} style={{ padding:"10px 12px", background:hi?accent:"#f8fafc", borderRadius:"8px", border:`1px solid ${hi?color+"33":"#f1f5f9"}` }}>
      ...
      <div style={{ marginBottom:"18px" }}>
        ...
        {path.nodes.map((node, i) => {
          ...
          const elapsed = node.state.monthsElapsed, duration = +(elapsed-prevElapsed).toFixed(1)
          const moves = node.moves ?? []
          return (
            <div key={i} style={{ display:"flex", alignItems:"center", gap:"8px", padding:"7px 10px", marginBottom:"4px", background:"#f8fafc", borderRadius:"6px", borderLeft:`3px solid ${color}` }}>
              <div style={{ display:"flex", gap:"1px", flexShrink:0 }}>{moves.map((m,mi) => <span key={mi} style={{ fontSize:"12px" }}>{m.icon}</span>)}</div>
              <div style={{ flex:1, minWidth:0 }}>{moves.map((m,mi) => <div key={mi} style={{ fontSize:"11px", fontWeight:"600", color:"#0f172a", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{m.name}</div>)}</div>
      ...
      {isFront && (
        <button onClick={e => { e.stopPropagation(); onDetail() }}
          style={{ width:"100%", padding:"10px", background:color, border:"none", borderRadius:"8px", color:"white", fontSize:"11px", fontWeight:"700", letterSpacing:"1.5px", cursor:"pointer", fontFamily:"monospace" }}>
          VIEW FULL DETAIL →
        </button>
      )}
    </div>
                ...
                return (
                  <tr key={i} style={{ borderBottom:"1px solid #f8fafc", verticalAlign:"top" }}>
                    ...
                    <td style={{ padding:"12px 8px 12px 0", whiteSpace:"nowrap" }}>
                      ...
                      <div style={{ fontSize:"10px", color:cashDelta>=0?"#16a34a":"#ef4444", fontFamily:"monospace" }}>{cashDelta>=0?"+":""}{fmt(cashDelta)}</div>
                    </td>
...
function GoalsPanel({ goals, setGoals, risk, setRisk, onClose, pathCount }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:100, display:"flex", alignItems:"flex-start", justifyContent:"flex-end" }}>
      ...
      <div style={{ position:"relative", width:"360px", height:"100vh", background:"white", borderLeft:"1px solid #e2e8f0", padding:"32px 28px", overflowY:"auto", boxShadow:"-8px 0 40px rgba(0,0,0,0.08)", animation:"slideIn 0.25s ease" }}>
        ...
        <div style={{ marginBottom:"28px" }}>
          ...
          <div style={{ padding:"12px 14px", background:"#faf5ff", borderRadius:"8px", border:"1px solid #e9d5ff", fontSize:"11px", color:"#7c3aed", lineHeight:"1.6" }}>
            {risk < 33 ? "Penalizing strategies that dip below safe cash levels." : risk < 66 ? "Balanced approach. Moderate cash dips acceptable." : "Prioritizing upside. Risky strategies score well if returns are high."}
          </div>
        </div>
        <div style={{ height:"1px", background:"#f1f5f9", marginBottom:"24px" }} />
        <div>
...
export default function Results({ result, initialState, isLoading }) {
  ...
  if (showAnimation && !readyToShow) {
    return (
      <div style={{ minHeight:"calc(100vh - 56px)", background:"#fafafa", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"20px", padding:"40px 24px" }}>
        <div style={{ textAlign:"center", marginBottom:"4px" }}>
          <div style={{ fontSize:"10px", color:"#94a3b8", letterSpacing:"3px", fontFamily:"monospace", marginBottom:"10px" }}>AUGUR ENGINE RUNNING</div>
        ...
        <div style={{ display:"flex", gap:"6px", alignItems:"center" }}>
          {[0,1,2].map(i => (
            <div key={i} style={{ width:6, height:6, borderRadius:"50%", background: dataReady ? "#16a34a" : "#cbd5e1", animation:`pulse 1.2s ease-in-out ${i*0.2}s infinite` }} />
          ))}
  ...
  const getOffset = i  => { const d = i - active; return d===0?0:(d===1||d===-2)?1:-1 }
